...
str(earthquakes) #### Nie wiem czy to chcemy bo wygląda trochę obleśnie, ale mówi jakie są typy danych w datasecie, jak fujka to usuń, jak nie fujka to opiszemy :)))
```
...
```{r echo=FALSE, fig.cap="\\label{barplot1}Wykres słupkowy dla 12 najbardziej narażonych miejsc"}
...
```{r corrpl, echo=FALSE, fig.cap="\\label{corrpl}Macierz korelacji coś tam coś tam"}
...
```{r, echo=FALSE, fig.cap="\\label{depth_mag}Scatterplot zależności siły trzęsienia od jego głębokości"}
