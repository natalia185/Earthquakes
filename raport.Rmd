...
Po pierwsze odpowiadając na pytanie badawcze postawione na początku możemy stwierdzić, że najbardziej narażone na występowanie trzęsień ziemi były regiony azjatyckie, Oceania, jak również zachodnie wybrzeża Ameryki Południowej oraz Ameryki Północnej (\ref{world}). Stąd wniosek, że to właśnie te regiony są najbardziej narażone. Jeśli jednak spojrzymy na konkretne państwa to zgodnie z wykresem \ref{barplot1} w czołowej trójce plasują się takie kraje jak Indonezja, Papua-Nowa Gwinea i Japonia. Warto tutaj zaznaczyć, że wyniki te mogą być delikatnie zakłamane `(wtf... brzmi dziko)` ze względu na ilość stacji sejsmicznych użytych do zlokalizowania zdarzenia. Zgodnie z wykresem \ref{nst_years} z biegiem lat ośrodków takich przybywało, co pozwalało dokładniej określić położenie niż w latach wcześniejszych. Jednak nie uważamy, aby miało to znaczący wpływ na otrzymane wyniki.`(Czy ja właśnie sobie zaprzeczyłam? Być może XD)`

Idąc dalej, zgodnie z macierzą korelacji \ref{corrpl} jak i późniejszą analizą nie widzimy znaczących korelacji w danych. Co może zaskakiwać głębokość odnotowanego zdarzenia nie jest znacząco zależna od jego siły, można powiedzieć, że jest to nieintuicyjny wynik. `(włączył mi się polski, ok?)`

Badając średnią siłę trzęsienia (tabela \ref{tabela2}) i porównując ją dla 12 najbardziej narażonych miejsc (wykres \ref{barplot1}) możemy stwierdzić, że średnie wartości są do siebie zbiżone. Dodatkowo korelacja dla wszystkich danych jest bliska zera `(Tylko co nam to daje?)`

Kolejne analizy nie wykazały żadnych zależności związanych z czasem czy też porą roku, zatem możemy stąd wywnioskować, że czynniki te nie mają wpływu na badane zjawisko.

Reasumując nasze rozważania trzęsienia ziemi są zjawiskiem nieprzewidywalnym i zależnym w głównej mierze od położenia geograficznego. Można przypuszczać, że zaczęły występować częśćj niż miało to miejsce w poprzednich latach, jednak nie możemy tego bezpośrednio stwierdzić z uwagi na liczbę stacji badawczych, których liczba powiększała się, a co za tym idzie obszar pomiarów również ulegał zwiększeniu. Warto też zaznaczyć, że dane poddane analizie opisywały trzęsienia o magnitudzie powyżej 6, czyli nasze wnioski nie muszą być prawdziwe w ogólnym przypadku. `(O ile w ogóle są XD)`
